...
export async function POST(request) {
  try {
    ...
    qsos = qsos.map(q => {
      if (q.call_sign) {
        q.call_sign = q.call_sign.toUpperCase();
      }
      return q;
    });
    ...
    if (validQsos.length === 0) {
      return errorResponse(
        `没有有效的QSO记录（已跳过 ${invalidIndices.length} 条无效记录），请检查文件格式是否为标准ADIF`,
        'VALIDATION_FAILED',
        400
      );
    }
    ...
    // 强制添加 user_id
    const toInsert = uniqueQsos.map(q => ({ ...q, user_id: user.id }));
    ...
    if (error) {
      ...
      logError('POST /api/qso/logs/batch insert error', error);
    ...
    return successResponse(
      {
        ...
        duplicates: duplicateCount,
        skipped: invalidIndices.length,
        message: `成功导入 ${data.length} 条新QSO，跳过 ${duplicateCount} 条重复记录，跳过 ${invalidIndices.length} 条无效记录`,
      },
      'Import completed'
    );
  } catch (err) {
...
export async function DELETE(request) {
  try {
    ...
    if (error) {
      logError('DELETE /api/qso/logs/batch error', error);
      return errorResponse(error.message, 'DB_ERROR', 500);
    }
